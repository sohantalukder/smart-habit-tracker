...
export async function GET(request: Request) {
  ...
  const upstream = await serverApiRequest("/auth/verify-email-change", {
    method: "POST",
  ...
  const response = NextResponse.redirect(
    new URL("/dashboard/profile?emailChanged=true", request.url),
  );
...
function failureRedirect(requestUrl: string) {
  return NextResponse.redirect(
    new URL("/login?emailChange=invalid", requestUrl),
  );
}
